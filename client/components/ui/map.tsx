...
> = {
  ...
  pointColor?: string;
  /**
   * Optional paint expression for unclustered point color. Takes precedence
   * over `pointColor` when provided, allowing per-feature coloring via a
   * feature property (e.g. availability bucket).
   */
  pointColorExpression?: MapLibreGL.ExpressionSpecification;
  /**
   * Numeric feature id (from `generateId`) of the currently hovered point,
   * used to highlight it via `setFeatureState`. Requires `generateId` to be
   * enabled on the underlying source (this component always does so).
   */
...
>({
  ...
  // Tracks the previously-hovered numeric feature id so we can clear its
  // feature-state when the hover target changes.
  ...
  useEffect(() => {
    ...
    map.addSource(sourceId, {
      ...
      clusterMaxZoom,
      clusterRadius,
      // Numeric IDs are auto-assigned per feature so `setFeatureState` for
      // hover highlight can target them by id.
      generateId: true,
    });
    ...
    map.addLayer({
      ...
      filter: ["!", ["has", "point_count"]],
      paint: {
        "circle-color": pointColorExpression ?? pointColor,
        "circle-radius": pointColorExpression ? 7 : 5,
        "circle-stroke-width": [
          "case",
          ["boolean", ["feature-state", "hover"], false],
          3,
  ...
  useEffect(() => {
    ...
    if (
      ...
      (prev.pointColor !== pointColor ||
        prev.pointColorExpression !== pointColorExpression)
    ) {
      map.setPaintProperty(
        unclusteredLayerId,
        "circle-color",
        pointColorExpression ?? pointColor,
      );
    }
  ...
  // Apply hover feature-state to the currently hovered point. The unclustered
  // layer's paint expression reads `feature-state.hover` to thicken the
  // stroke and switch it to the brand teal. We clear the previous id first
  // because MapLibre doesn't auto-clear hover on a different target.
  useEffect(() => {
    if (!isLoaded || !map) return;
    if (
      ...
      try {
        map.setFeatureState(
          { source: sourceId, id: previousHoveredIdRef.current },
          { hover: false },
        );
      } catch {
        /* feature id may have been recycled by a data update — ignore */
      }
    }
    if (hoveredId !== null) {
      try {
    ...
    previousHoveredIdRef.current = hoveredId;
  }, [hoveredId, isLoaded, map, sourceId]);
